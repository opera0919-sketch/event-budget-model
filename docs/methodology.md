...
```
- 실적 보정값(`data/reference_deposit_events.csv`, 10회차): **전환율 62% · 조건충족률 45% → 지급률 27.8%**,
...
당첨자 금액 분포는 **가정하지 않고 실측을 쓴다**. 이벤트 10회차의 타사수관금액 구간별
**고객 수**가 데이터로 있고(총 신청 100,058명, 리워드 대상 19,920명), 이를 고객 수로
가중해 통합한 분포를 기본으로 삼는다. 회차 규모가 4,063~19,831명으로 5배 차이나므로
가중이 맞다(회차 단순평균을 쓰면 지급률이 21.22%로 올라간다).

모집단이 '신청 고객'임은 교차 검증됐다 — 회차별 총 고객 수가
...
축2의 눈금은 **실측 회차 간 변동 그 자체**다. `theta=0`은 10회차 고객 수 가중 통합,
...
(단가 ±9% vs 지급률 13.7~31.7% ±40%). 그래서 관측된 변동 방향을 직접 쓰는
편이 가정을 덜 넣고 더 정확하다.
...
몬테카를로는 신청배수 삼각·기울기 삼각·지급률 잔여 잡음을 결합한다. 밴드는 **관측 범위
안으로 제한**한다 — 기울기 `-1.0 ~ +1.0`(실측 최저~최고 그 자체), 신청 배수 `0.8 ~ 1.2`.
관측 범위를 넘는 극단(기울기 +1.5, 배수 1.8)은 결정론 매트릭스의 worst case 셀이 담당한다.
편성 권고치(P90)를 외삽 구간까지 끌어올리면 상시 과대편성이 되므로 역할을 나눈 것이다.
밴드는 `stress.mc_shift_band`·`stress.mc_mult_band`(명세) 또는 CLI
`--mc-shift-band`·`--mc-mult-band`로 바꿀 수 있다.
